...
EXCEL_PATH = Path(__file__).parent / "Analiza_Piata_Profesionala.xlsx"
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(
            Path(__file__).parent / "actualizare.log", encoding="utf-8"
        ),
    ]
)
...
# ══════════════════════════════════════════════════════════════════════════════
# CREARE EXCEL TEMPLATE (dacă fișierul nu există)
# ══════════════════════════════════════════════════════════════════════════════

def style_header_row(ws, row, num_cols, title_fill=None, title_font=None):
    hdr_fill = title_fill or F_HDR
    hdr_font = title_font or FN_HDR
...
def create_excel_template(path):
    log.info(f"  Creez fisierul Excel: {path}")
    wb = Workbook()

    # ── 1. PRETURI VOLUME ──────────────────────────────────────────────────────
    ws = wb.active
    ws.title = "PRETURI VOLUME"
    ws.sheet_properties.tabColor = "1F4E79"
    headers_pv = [
        "Data", "Activ", "Ticker",
        "Deschidere", "Maxim", "Minim", "Inchidere",
        "Var.Zi%", "Var.Sapt%", "Var.Luna%",
        "Volum", "AvgVol 20z", "RVOL",
        "Semnal", "Trend",
    ]
    ws.merge_cells("A1:O1")
    ws["A1"] = "PRETURI SI VOLUME — Actualizare Automata"
    ws["A1"].fill = fill("0D47A1")
    ws["A1"].font = fnt(bold=True, color="FFFFFF", size=14)
    ws["A1"].alignment = ALIGN_CENTER
    for i, h in enumerate(headers_pv, 1):
        ws.cell(2, i, h)
    style_header_row(ws, 2, len(headers_pv))
    col_widths_pv = [12, 18, 14, 14, 14, 14, 14, 10, 10, 10, 14, 14, 8, 10, 12]
    for i, w in enumerate(col_widths_pv, 1):
        ws.column_dimensions[get_column_letter(i)].width = w
    ws.freeze_panes = "A3"

    # ── 2. INDICATORI TEHNICI ──────────────────────────────────────────────────
    ws2 = wb.create_sheet("INDICATORI TEHNICI")
    ws2.sheet_properties.tabColor = "2E7D32"
    headers_it = [
        "Data", "Activ", "Ticker", "Inchidere",
        "MA20", "MA50", "MA200",
        "RSI", "RSI Status",
        "MACD", "MACD Signal", "MACD Hist", "MACD Cross",
        "BB Sup", "BB Inf", "BB Width",
        "ATR",
        "Stoch %K", "Stoch %D",
        "Volum", "RVOL",
        "Trend", "MA Cross",
    ...
    ws2["A1"] = "INDICATORI TEHNICI — Analiza Completa"
    ws2["A1"].fill = fill("1B5E20")
    ws2["A1"].font = fnt(bold=True, color="FFFFFF", size=14)
    ws2["A1"].alignment = ALIGN_CENTER
    for i, h in enumerate(headers_it, 1):
        ws2.cell(2, i, h)
    style_header_row(ws2, 2, len(headers_it))
    col_widths_it = [12, 18, 14, 14, 14, 14, 14, 8, 16, 12, 12, 12, 16, 14, 14, 12, 10, 10, 10, 14, 8, 12, 14]
    for i, w in enumerate(col_widths_it, 1):
        ws2.column_dimensions[get_column_letter(i)].width = w
    ws2.freeze_panes = "A3"

    # ── 3. SEMNALE INTRARE ─────────────────────────────────────────────────────
    ws3 = wb.create_sheet("SEMNALE INTRARE")
    ws3.sheet_properties.tabColor = "E65100"
    headers_si = [
        "Data", "Activ", "Semnal", "Conditie",
        "RSI", "MACD Cross", "MA Cross", "Volum",
        "Momentum 10z", "Confluente",
        "Pret Intrare", "Stop Loss", "Take Profit",
        "R:R", "Probabilitate",
        "Status", "Ora Actualizare",
    ]
    ws3.merge_cells("A1:Q1")
    ws3["A1"] = "SEMNALE DE INTRARE — Trading Signals"
    ws3["A1"].fill = fill("BF360C")
    ws3["A1"].font = fnt(bold=True, color="FFFFFF", size=14)
    ws3["A1"].alignment = ALIGN_CENTER
    for i, h in enumerate(headers_si, 1):
        ws3.cell(2, i, h)
    style_header_row(ws3, 2, len(headers_si))
    col_widths_si = [12, 18, 10, 40, 8, 16, 14, 10, 12, 12, 14, 14, 14, 8, 12, 10, 16]
    for i, w in enumerate(col_widths_si, 1):
        ws3.column_dimensions[get_column_letter(i)].width = w
    ws3.freeze_panes = "A3"

    # ── 4. INDICATORI MACRO ────────────────────────────────────────────────────
    ws4 = wb.create_sheet("INDICATORI MACRO")
    ws4.sheet_properties.tabColor = "4A148C"
    headers_macro = [
        "Indicator", "Valoare Actuala", "Valoare Anterioara",
        "Variatie", "Nivel", "Interpretare",
        "Impact Piata", "Sursa",
        "Ultima Actualizare",
    ]
    ws4.merge_cells("A1:I1")
    ws4["A1"] = "INDICATORI MACROECONOMICI"
    ws4["A1"].fill = fill("311B92")
    ws4["A1"].font = fnt(bold=True, color="FFFFFF", size=14)
    ws4["A1"].alignment = ALIGN_CENTER
    for i, h in enumerate(headers_macro, 1):
        ws4.cell(2, i, h)
    style_header_row(ws4, 2, len(headers_macro))

    macro_indicators = [
        ("VIX (Volatility Index)", None, None, None, None, "Masoara volatilitatea asteptata S&P 500", "Invers pe actiuni", "Yahoo Finance"),
        ("Yield 10Y US (Treasury)", None, None, None, None, "Randament obligatiuni SUA 10 ani", "Invers pe actiuni/crypto", "Yahoo Finance"),
        ("Yield 2Y US (Treasury)", None, None, None, None, "Randament obligatiuni SUA 2 ani", "Indicator politica monetara", "Yahoo Finance"),
        ("Yield 30Y US (Treasury)", None, None, None, None, "Randament obligatiuni SUA 30 ani", "Indicator inflatie pe termen lung", "Yahoo Finance"),
        ("USD Index (DXY)", None, None, None, None, "Puterea dolarului vs cos valute", "Invers pe aur/crypto/EM", "Yahoo Finance"),
        ("Fear & Greed Index", None, None, None, None, "Sentiment general piata crypto", "Direct pe crypto", "Alternative.me"),
        ("Rata dobanzii Fed (Fed Funds)", None, None, None, None, "Rata de referinta a Fed", "Major pe toate pietele", "FRED"),
        ("CPI (Consumer Price Index)", None, None, None, None, "Indicator principal inflatie", "Inflatie crescuta = presiune pe actiuni", "FRED"),
        ("Somaj US (Unemployment)", None, None, None, None, "Rata somajului in SUA", "Somaj crescut = risc recesiune", "FRED"),
    ]
    for idx, row_data in enumerate(macro_indicators, 3):
        for col, val in enumerate(row_data, 1):
            if val is not None:
                cell = ws4.cell(idx, col, val)
                if col == 1:
                    cell.font = fnt(bold=True)
    col_widths_macro = [32, 16, 18, 12, 12, 40, 28, 16, 20]
    for i, w in enumerate(col_widths_macro, 1):
        ws4.column_dimensions[get_column_letter(i)].width = w
    ws4.freeze_panes = "A3"

    # ── 5. DASHBOARD ─────────────────────────────────────────────────────────
    ws5 = wb.create_sheet("DASHBOARD")
    ws5.sheet_properties.tabColor = "006064"
    ws5.merge_cells("A1:H1")
    ws5["A1"] = "DASHBOARD — Rezumat Piata"
    ws5["A1"].fill = fill("004D40")
    ws5["A1"].font = fnt(bold=True, color="FFFFFF", size=14)
    ...
    dash_labels = [
        ...
        (2, 2, "Valoare"),
        (2, 3, "Detalii"),
        (2, 4, "Ultima Actualizare"),
        (2, 5, ""),
        (2, 6, "Categorie"),
        (2, 7, "Valoare"),
        (2, 8, "Detalii"),
    ]
    for r, c, v in dash_labels:
        ws5.cell(r, c, v)
    style_header_row(ws5, 2, 8)

    dash_rows = [
        (3, 1, "Total Active Monitorizate"),
        (4, 1, "Semnale BUY"),
        (5, 1, "Semnale SELL"),
        (6, 1, "Semnale WAIT"),
        (7, 1, "% BUY"),
    ...
    for r, c, v in dash_rows:
        cell = ws5.cell(r, c, v)
        if v:
            cell.font = fnt(bold=True)
    col_widths_dash = [28, 14, 24, 20, 4, 20, 14, 24]
    for i, w in enumerate(col_widths_dash, 1):
        ws5.column_dimensions[get_column_letter(i)].width = w
    ws5.freeze_panes = "A3"

    # ── 6. ISTORIC TRENDING ────────────────────────────────────────────────────
    ws6 = wb.create_sheet("ISTORIC TRENDING")
    ws6.sheet_properties.tabColor = "880E4F"
    headers_ist = [
        "Luna / An", "RSI Mediu", "Trend Dominant",
        "S&P 500", "NASDAQ", "Bitcoin",
        "EUR/USD", "Semnal Luna",
        "Nr BUY", "Nr SELL",
        "Fear & Greed", "Observatii",
    ]
    ws6.merge_cells("A1:L1")
    ws6["A1"] = "ISTORIC TRENDING — Evolutie Lunara"
    ws6["A1"].fill = fill("4A0025")
    ws6["A1"].font = fnt(bold=True, color="FFFFFF", size=14)
    ws6["A1"].alignment = ALIGN_CENTER
    for i, h in enumerate(headers_ist, 1):
        ws6.cell(2, i, h)
    style_header_row(ws6, 2, len(headers_ist))
    col_widths_ist = [14, 12, 16, 14, 14, 14, 12, 14, 10, 10, 14, 24]
    for i, w in enumerate(col_widths_ist, 1):
        ws6.column_dimensions[get_column_letter(i)].width = w
    ws6.freeze_panes = "A3"

    wb.save(str(path))
    log.info(f"  Fisierul Excel a fost creat cu succes: {path.name}")
    log.info(f"  Sheet-uri: {wb.sheetnames}")
...
def calc_signal(rsi, macd_cross, ma_cross, rvol):
    ...
    if "Bullish Cross" in macd_cross:
        score += 2
    elif "Pozitiv" in macd_cross:
        score += 1
    elif "Bearish Cross" in macd_cross:
        score -= 2
    elif "Negativ" in macd_cross:
        score -= 1
    ...
    if score >= 3:
        return "BUY", confluente
    elif score <= -3:
        return "SELL", confluente
    else:
        return "WAIT", confluente
...
def get_full_data(name, ticker):
    try:
        ...
        price = close_now
        if ma["ma50"] and price > ma["ma50"] * 1.01:
            trend = "Bullish"
        elif ma["ma50"] and price < ma["ma50"] * 0.99:
            trend = "Bearish"
        else:
            trend = "Sideways"

        rsi_status = (
            "Supracumparat ▼" if rsi > 70 else
...
def color_pnl(cell, value):
    if value > 0:
        cell.fill = F_GRN; cell.font = FN_GRN
    elif value < 0:
        cell.fill = F_RED; cell.font = FN_RED
    else:
        cell.fill = F_YLW; cell.font = FN_YLW
        ...
        for name, ticker in cat_dict.items():
            ...
            if d:
                ...
                if d["macross"] == "Golden Cross":
                    mc.fill = F_GRN; mc.font = FN_GRN
                elif d["macross"] == "Death Cross":
                    mc.fill = F_RED; mc.font = FN_RED
                else:
                    mc.fill = F_YLW; mc.font = FN_YLW
            r += 1
...
def update_semnale(ws, all_data):
    ...
    for cat_name, cat_dict in CATEGORII:
        ...
        for name, ticker in cat_dict.items():
            ...
            vol_text = (
                "Crescut" if d["rvol"] > 1.3 else
                ("Scazut" if d["rvol"] < 0.8 else "Normal")
            )
            ...
            if d["probabilitate"] >= 65:
                prob_c.fill = F_GRN; prob_c.font = FN_GRN
            elif d["probabilitate"] >= 50:
                prob_c.fill = F_YLW; prob_c.font = FN_YLW
            else:
                prob_c.fill = F_RED; prob_c.font = FN_RED
            r += 1
...
def update_macro(ws, macro_live, fear_greed):
    ...
    macro_map = {
        ...
        "Yield 30Y":      macro_live.get("Yield 30Y US", {}).get("inchidere"),
    ...
    for r in range(3, 50):
        ...
        for key, val in macro_map.items():
            if key.lower() in str(ind).lower() and val is not None:
                ...
                # Variatie (col 4)
                prev_val = ws.cell(r, 3).value
                if prev_val is not None:
                    try:
                        pv = float(str(prev_val).replace(",", "."))
                        if pv != 0:
                            variatie = round((float(val) - pv) / pv * 100, 2)
                            ws.cell(r, 4).value = variatie
                            ws.cell(r, 4).number_format = "0.00%"
                            color_pnl(ws.cell(r, 4), variatie)
                    except (ValueError, TypeError):
                        pass

                # Nivel (col 5)
                if "vix" in str(ind).lower():
                    if float(val) > 30:
                        ws.cell(r, 5).value = "Ridicat"
                        ws.cell(r, 5).fill = F_RED; ws.cell(r, 5).font = FN_RED
                    elif float(val) > 20:
                        ws.cell(r, 5).value = "Moderat"
                        ws.cell(r, 5).fill = F_YLW; ws.cell(r, 5).font = FN_YLW
                    else:
                        ws.cell(r, 5).value = "Scazut"
                        ws.cell(r, 5).fill = F_GRN; ws.cell(r, 5).font = FN_GRN
                elif "fear" in str(ind).lower():
                    if float(val) > 60:
                        ws.cell(r, 5).value = "Lacomie"
                        ws.cell(r, 5).fill = F_GRN; ws.cell(r, 5).font = FN_GRN
                    elif float(val) < 40:
                        ws.cell(r, 5).value = "Frica"
                        ws.cell(r, 5).fill = F_RED; ws.cell(r, 5).font = FN_RED
                    else:
                        ws.cell(r, 5).value = "Neutru"
                        ws.cell(r, 5).fill = F_YLW; ws.cell(r, 5).font = FN_YLW
...
def update_dashboard(ws, all_data, fear_greed):
    ...
    total = buy + sell + wait
    pct_buy = round(buy / total * 100, 1) if total > 0 else 0
    pct_sell = round(sell / total * 100, 1) if total > 0 else 0
    trend = (
        "Bullish" if pct_buy > 55 else
        ("Bearish" if pct_sell > 55 else "Mixt / Neutru")
    )

    ws.cell(3, 2, total)
    ws.cell(4, 2, buy);   color_signal(ws.cell(4, 2), "BUY")
    ws.cell(5, 2, sell);  color_signal(ws.cell(5, 2), "SELL")
    ws.cell(6, 2, wait);  color_signal(ws.cell(6, 2), "WAIT")
    ws.cell(7, 2, pct_buy / 100);  ws.cell(7, 2).number_format = "0.0%"
    ws.cell(8, 2, pct_sell / 100); ws.cell(8, 2).number_format = "0.0%"
    ws.cell(9, 2, trend);  color_trend(ws.cell(9, 2), trend)
    ws.cell(10, 2, fear_greed.get("display", "N/A"))
    fg_val = fear_greed.get("value")
    if fg_val is not None:
        if fg_val > 60:
            ws.cell(10, 2).fill = F_GRN; ws.cell(10, 2).font = FN_GRN
        elif fg_val < 40:
            ws.cell(10, 2).fill = F_RED; ws.cell(10, 2).font = FN_RED
        else:
            ws.cell(10, 2).fill = F_YLW; ws.cell(10, 2).font = FN_YLW

    ws.cell(3, 3, f"BUY:{buy} SELL:{sell} WAIT:{wait}")
    ws.cell(9, 3, f"BUY {pct_buy}% vs SELL {pct_sell}%")
    ...
    key_tickers = {
        ...
        8: ("VIX",          "^VIX"),
        9: ("Oil WTI",      "CL=F"),
        10: ("USD Index",   "DX-Y.NYB"),
    }
    for row, (name, ticker) in key_tickers.items():
        d = all_data.get(ticker, {})
        if d:
            ws.cell(row, 7, d["inchidere"])
            ws.cell(row, 7).number_format = "#,##0.0000"
            detail = f"{'+'if d['var_zi_pct'] >= 0 else ''}{d['var_zi_pct']:.2f}% | {d['semnal']}"
            ws.cell(row, 8, detail)
            color_pnl(ws.cell(row, 8), d["var_zi_pct"])

    if total > 0:
        log.info(
            f"  OK Dashboard — BUY:{buy}({pct_buy}%) "
...
def update_historic(ws, all_data, fear_greed):
    ...
    trend_vals = [d["trend"] for d in all_data.values() if d and d.get("trend")]
    bullish_count = sum(1 for t in trend_vals if t == "Bullish")
    bearish_count = sum(1 for t in trend_vals if t == "Bearish")
    trend_dominant = (
        "Bullish" if bullish_count > bearish_count else
        ("Bearish" if bearish_count > bullish_count else "Mixt")
    )

    ws.cell(r, 1, luna_an)
    ws.cell(r, 2, avg_rsi)
    ws.cell(r, 3, trend_dominant)
    color_trend(ws.cell(r, 3), trend_dominant)
    ...
    nasdaq = all_data.get("^NDX", {}).get("inchidere")
    if nasdaq:
        ws.cell(r, 5, nasdaq)
        ws.cell(r, 5).number_format = "#,##0.00"

    btc = all_data.get("BTC-USD", {}).get("inchidere")
    if btc:
        ws.cell(r, 6, btc)
        ws.cell(r, 6).number_format = "#,##0.00"

    eurusd = all_data.get("EURUSD=X", {}).get("inchidere")
    if eurusd:
        ws.cell(r, 7, eurusd)
    ...
    if fear_greed.get("value"):
        ws.cell(r, 11, fear_greed["value"])

    log.info(
        f"  OK Istoric — adaugat {luna_an} RSI={avg_rsi} Semnal={semnal_luna}"
    )
...
def main():
    ...
    if not EXCEL_PATH.exists():
        log.info(f"  Fisierul Excel nu exista. Se creeaza: {EXCEL_PATH}")
    ...
    for i, (name, ticker) in enumerate(ACTIVE.items(), 1):
        ...
        if data:
            ...
            log.warning(
                f"  [{i:3d}/{len(ACTIVE)}] {name:25s} -- EROARE / N/A --"
            )
    ...
    log.info(
        f"  Procesate: {ok}/{len(ACTIVE)} active OK"
